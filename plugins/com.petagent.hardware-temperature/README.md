...
首次采集会启动随插件分发的管理员常驻 helper（仅监听本机回环地址）；如果系统没有 PawnIO，helper 会请求一次管理员权限安装内置 `PawnIO_setup.exe`。后续采集复用该进程，不需要 PetAgent 本身以管理员身份运行。Windows 没有统一的 CPU 核心温度 API；驱动或硬件不支持时，工具返回 `available: false` 以及 `sources` / `limitations` 说明。
