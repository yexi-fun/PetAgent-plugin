...
- NVIDIA 温度优先来自 NVML（NVML 成功时过滤同一 NVIDIA 硬件的 WMI 重复读数）；CPU 和 AMD/Intel GPU 温度来自 LibreHardwareMonitor/OpenHardwareMonitor WMI，并通过 `Sensor.Parent` 关联 `Hardware.HardwareType` 后按 NexBox 传感器名称筛选。
