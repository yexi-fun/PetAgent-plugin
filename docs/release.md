...
3. Run `scripts/publish-market.ps1 -Tag <tag> -SkipRelease` to compute optional
   payload/ZIP metadata and regenerate the reviewed-repository index.
...
No signing key or signature configuration is required; install only sources you
trust because plugins execute with host permissions.
